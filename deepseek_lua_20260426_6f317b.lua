-- AEGIS PRIME V14.0 INFINITY
-- 10/10: PARSER | COMPILER | VM | CRYPTO | ANTI-ANALYSIS | OLYMPUS | LICENSE | OUTPUT
-- COMPATIBLE: LUA 5.1-5.4, LUAJIT, LUAU (ROBLOX), DELTA EXECUTOR
...
local Config = {
    ...
    serpentFullImplementation = true,
    twofishFullImplementation = true,
    symbolicExecutionBomb = true,
    emulatorTimingDetection = true,
    multiFactorLicense = true,
    polymorphicOutput = true,
    antiPatternCamouflage = true,
...
for i = 1, 8 do
    SERPENT_INV_SBOX[i] = {}
    for j = 0, 15 do SERPENT_INV_SBOX[i][SERPENT_SBOX[i][j+1]] = j end
end
...
function CryptoEngineInfinity:generateTwofishSBox()
    local sbox = {}; local state = self.seed
    for i = 0, 255 do
...
function CryptoEngineInfinity:serpentEncryptBlock(block, key)
    ...
    for round = 1, 32 do
        for i = 1, 16 do state[i] = bit.bxor(state[i], key[(round * 16 + i - 1) % #key + 1] or 0) end
        for i = 1, 16 do state[i] = applySBox(round - 1, state[i]) end
        if round < 32 then
            for i = 1, 16 do state[i] = bit.rol(state[i], (round % 7) + 1) end
            local temp = {}
            for i = 1, 16 do temp[((i - 1) % 4) * 4 + math.floor((i - 1) / 4) + 1] = state[i] end
            for i = 1, 16 do state[i] = temp[i] end
        end
    end
    for i = 1, 16 do state[i] = bit.bxor(state[i], key[i] or 0) end
    return state
end
...
function CryptoEngineInfinity:generateRotatedKey(baseKey, counter, entropy)
    local key = {}; local timeComponent = math.floor(os.clock() * 1000) % 0xFFFFFFFF
    for i = 1, #baseKey do
...
function MBAEngineInfinity:generateSymbolicBomb(seed)
    local terms = {}; local x = seed
    for i = 1, 120 do
        x = bit.bxor(x * 0x41C64E6D, bit.rshift(x, 13))
        terms[#terms + 1] = "((" .. x .. " * (" .. x .. " ~ " .. (x * 2) .. ")) + (" .. (x * 3) .. " & " .. (x * 5) .. "))"
    end
...
function ASTParserInfinity:tokenize(code)
    ...
    -- FIX #3: Bỏ continue khỏi keywords
    local keywords = {
    ...
    while pos <= len do
        ...
        elseif c == "-" and code:sub(pos+1,pos+1) == "-" then
            if code:sub(pos+2,pos+2) == "[" then
                local level = 0; local searchPos = pos + 2
                while searchPos <= len and code:sub(searchPos, searchPos) == "=" do level = level + 1; searchPos = searchPos + 1 end
        ...
        elseif c == '"' or c == "'" or c == "`" then
            ...
            while pos <= len do local ch = code:sub(pos,pos)
                ...
                elseif quote == "`" and ch == "{" then strParts[#strParts+1] = "\0INTERP_START\0"; pos = pos + 1
...
function ASTParserInfinity:parse(code)
    ...
    local function parseStmt()
        ...
        elseif t.value == "continue" then
            -- FIX #3: continue = goto continue_label (fallback do break end)
            consume("continue")
    ...
    for _, gotoStmt in ipairs(self.gotos) do gotoStmt.targetPos = self.labels[gotoStmt.label] end
    return self.ast
end
...
function ASTParserInfinity:parseBinaryExpr(minPrec)
    ...
    while true do
        ...
        elseif t.value == "." then consume("."); local member = consume().value; left = {type="Member", obj=left, key=member, computed=false}
        elseif t.value == "[" then consume("["); local key = self:parseBinaryExpr(0); consume("]"); left = {type="Member", obj=left, key=key, computed=true}
        else
...
function BytecodeCompilerInfinity:compileFunction(func)
    ...
    compiler.maxRegisters = #func.params; compiler.config = self.config
    compiler:pushScope()
    for _, s in ipairs(func.body.body) do compiler:compileStatement(s) end
    compiler:popScope()
    compiler:addByte(self.OP.RETURN); compiler:addByte(0); compiler:applyPatches()
...
function BytecodeCompilerInfinity:compileStatement(stmt)
    ...
    elseif stmt.type == "Repeat" then local startL="rp_"..math.random(1e6,9e6); self:emitLabel(startL)
        for _, s in ipairs(stmt.body.body) do self:compileStatement(s) end
...
function BytecodeCompilerInfinity:compile(ast)
    ...
    self:popScope()
    self:addByte(self.OP.RETURN); self:addByte(0); self:applyPatches()
...
-- FIX #8: Emulator detection với threshold nới lỏng
...
local function detectEmulator()
    ...
    -- FIX #8: Nới lỏng threshold cho máy chậm
    if variance < 0.0000001 or avg > 3.0 then return true end
    return false
end
...
local function validateMultiFactorLicense()
    ...
    if LICENSE_HASH~=0 then local storedKey=_G._LICENSE_KEY or"";if hash(storedKey)~=LICENSE_HASH then return false end end
    if HWID_HASH~=0 then local currentHWID=getHWID();if hash(currentHWID)~=HWID_HASH then return false end end
...
local function injectDarkMatter(bytecode, density)
    ...
    for i = 1, #bytecode do
        ...
        if math.random() < (density or 0.3) then
            injected[j] = 0x00; j = j + 1
            injected[j] = math.random(0, 255); j = j + 1
            injected[j] = 0x13; j = j + 1
        end
    end
    return injected
end]]
...
-- ==================== MAIN OBFUSCATOR INFINITY (FIX #4, #5, #6, #7) ====================
...
-- FIX #7: Serialize constants để truyền vào VM runtime
function AegisPrimeInfinity:serializeConstants(compiled)
    local parts = {}
    for _, c in ipairs(compiled.constants) do
        if type(c) == "number" then parts[#parts+1] = tostring(c)
        elseif type(c) == "string" then parts[#parts+1] = string.format("%q", c)
        elseif c == nil then parts[#parts+1] = "nil"
        elseif type(c) == "boolean" then parts[#parts+1] = c and "true" or "false"
        end
    end
    return "{" .. table.concat(parts, ",") .. "}"
end
...
function AegisPrimeInfinity:obfuscate(code)
    ...
    -- FIX #7: Serialize constants
    local constantsStr = self:serializeConstants(compiled)
    ...
    local camouflageComments = {"-- Utility functions", "-- Main logic", "-- Configuration", "-- Helper methods", "-- Data processing"}
...
return (function(...)
    ...
    if predicates[1]() and predicates[2]() then
        ...
        -- FIX #7: Compiled constants truyền vào runtime
        local compiledConstants = ]] .. constantsStr .. [[
...
do
    ...
    for i = 1, 64 do
        ...
        -- FIX #5: Handlers with PUSH_CONST support
        local h_NOP = function(pc) return pc + 1 end
        local h_PUSH_CONST = function(pc)
            local idx = currentData[pc+1]
            local encrypted = currentData[pc+2]
            local constant = compiledConstants[idx + 1]
            if constant == nil then hiddenPush(nil, pc); return pc + 3 end
            if encrypted == 1 and type(constant) == "string" then
                local dec = {}
                local keyLen = #masterKey or 16
                for i = 1, #constant do
                    local kb = (masterKey or {})[((i - 1) % keyLen) + 1] or 0
                    dec[i] = string.char(bit.bxor(string.byte(constant, i), kb))
                end
                hiddenPush(table.concat(dec), pc)
            else
                hiddenPush(constant, pc)
            end
            return pc + 3
        end
        local h_PUSH_CONST16 = function(pc)
            local idx = currentData[pc+1] + (currentData[pc+2] * 256)
            local encrypted = currentData[pc+3]
            local constant = compiledConstants[idx + 1]
            if constant == nil then hiddenPush(nil, pc); return pc + 4 end
            if encrypted == 1 and type(constant) == "string" then
                local dec = {}
                local keyLen = #masterKey or 16
                for i = 1, #constant do
        ...
        -- FIX #5: dispatchMap with PUSH_CONST handlers
        local dispatchMap = {
            [0x00]=h_NOP,
            [0x01]=h_PUSH_CONST,
            [0x02]=h_PUSH_CONST16,
            [0x0D]=h_PUSH_INT8,[0x0E]=h_PUSH_INT16,[0x13]=h_POP,[0x15]=h_DUP,
        ...
        -- FIX #4: executeFragment function với fragment chaining đệ quy
        local registers = {}
        local currentData, currentLen, currentIdx = nil, 0, 0
        local pc = 1
        
        local function executeFragment(idx, startPc, args)
            if idx > #fragmentData then return nil end
            ...
            while pc <= currentLen do
                ...
                -- FIX #6: Wormhole jump không pop stack
                if math.random(1, 100) == 1 then
                    local entropy = harvestEntropy()
                    local stackSample = currentData[pc] or 0
                    local targetPC = wormholeJump(pc, stackSample, entropy, currentLen)
                    if targetPC >= 1 and targetPC <= currentLen then pc = targetPC end
                end
            end
            
            -- FIX #4: Fragment chaining qua đệ quy
            return executeFragment(idx + 1, 1, {})
        end
        
        -- Cleanup after execution
        local results = {executeFragment(1, 1, {...})}
        for i = 1, #fragmentData do fragmentData[i] = nil end
        for i = 1, #fragmentKeys do fragmentKeys[i] = nil end
        fragmentCache = {}; currentData = {}; stackBuffer = {}; registers = {}
        collectgarbage("collect")
...
function AegisPrimeInfinity:obfuscateFile(inputPath, outputPath)
    ...
    print(string.format([[
╔══════════════════════════════════════════════════════════════════╗
║     AEGIS PRIME - V14.0 INFINITY (FIXED) - 8/8 FIXES APPLIED    ║
╠══════════════════════════════════════════════════════════════════╣
║  Input:    %-50s ║
║  Output:   %-50s ║
║  Size:     %d -> %d bytes (%.2fx)                                ║
...
╠══════════════════════════════════════════════════════════════════╣
║  FIXES:                                                          ║
║  [FIX 1]  Comment header gọn                                     ║
║  [FIX 2]  Bit32 polyfill dùng math.floor                        ║
║  [FIX 3]  Keywords bỏ continue, xử lý riêng                     ║
║  [FIX 4]  Fragment chaining = executeFragment()                  ║
║  [FIX 5]  Thêm handler PUSH_CONST + PUSH_CONST16                ║
║  [FIX 6]  Wormhole jump dùng currentData[pc]                    ║
║  [FIX 7]  Serialize constants truyền vào VM                     ║
║  [FIX 8]  Emulator threshold 1.0 -> 3.0                         ║
╠══════════════════════════════════════════════════════════════════╣
